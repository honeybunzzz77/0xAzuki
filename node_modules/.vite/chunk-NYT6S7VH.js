    ...
    if (typeof value === "string") {
      ...
      const check = (key2, type, defaultValue) => {
        ...
        if (typeof value[key2] !== type) {
          logger2.throwArgumentError("invalid fixed format (" + key2 + " not " + type + ")", "format." + key2, value[key2]);
        }
        return value[key2];
      };
...
init_virtual_process_polyfill();
init_buffer();
var version3 = "abstract-provider/5.5.1";
...
init_virtual_process_polyfill();
init_buffer();
var version4 = "abstract-signer/5.5.0";
...
init_virtual_process_polyfill();
init_buffer();
var version8 = "signing-key/5.5.0";
...
init_virtual_process_polyfill();
init_buffer();
var version9 = "transactions/5.5.0";
...
function formatNumber(value, name2) {
  ...
  if (result.length > 32) {
    logger10.throwArgumentError("invalid length for " + name2, "transaction:" + name2, value);
  }
  return result;
}
...
var Formatter = class {
  constructor() {
    logger11.checkNew(new.target, Formatter);
...
var TypedDataEncoder = class {
  ...
  static hashDomain(domain) {
    ...
    for (const name2 in domain) {
      ...
      if (!type) {
        logger14.throwArgumentError(`invalid typed-data domain key: ${JSON.stringify(name2)}`, "domain", domain);
      }
...
function _fetchData(connection, body, processFunc) {
  ...
  logger16.assertArgument(attemptLimit > 0 && attemptLimit % 1 === 0, "invalid connection throttle limit", "connection.throttleLimit", attemptLimit);
  const throttleCallback = typeof connection === "object" ? connection.throttleCallback : null;
  const throttleSlotInterval = typeof connection === "object" && typeof connection.throttleSlotInterval === "number" ? connection.throttleSlotInterval : 100;
  logger16.assertArgument(throttleSlotInterval > 0 && throttleSlotInterval % 1 === 0, "invalid connection throttle slot interval", "connection.throttleSlotInterval", throttleSlotInterval);
  const headers = {};
  let url = null;
  const options = {
    method: "GET"
  ...
  if (typeof connection === "string") {
    ...
    if (connection == null || connection.url == null) {
      logger16.throwArgumentError("missing URL", "connection.url", connection);
    }
    url = connection.url;
    ...
    if (connection.user != null && connection.password != null) {
      if (url.substring(0, 6) !== "https:" && connection.allowInsecureAuthentication !== true) {
        logger16.throwError("basic authentication requires a secure https url", Logger.errors.INVALID_ARGUMENT, { argument: "url", url, user: connection.user, password: "[REDACTED]" });
      }
  ...
  if (dataMatch) {
    try {
      ...
      logger16.throwError("processing response error", Logger.errors.SERVER_ERROR, {
    ...
    const promise = new Promise(function(resolve, reject) {
      if (timeout) {
        timer2 = setTimeout(() => {
          ...
          timer2 = null;
          reject(logger16.makeError("timeout", Logger.errors.TIMEOUT, {
  ...
  const runningFetch = function() {
    return __awaiter6(this, void 0, void 0, function* () {
      for (let attempt = 0; attempt < attemptLimit; attempt++) {
        ...
        try {
          ...
          if (response == null) {
            ...
            logger16.throwError("missing response", Logger.errors.SERVER_ERROR, {
              requestBody: bodyify(options.body, flatHeaders["content-type"]),
              requestMethod: options.method,
              serverError: error,
              url
        ...
        if (allow304 && response.statusCode === 304) {
          ...
          runningTimeout.cancel();
          logger16.throwError("bad response", Logger.errors.SERVER_ERROR, {
      ...
      return logger16.throwError("failed response", Logger.errors.SERVER_ERROR, {
        requestBody: bodyify(options.body, flatHeaders["content-type"]),
        requestMethod: options.method,
        url
      });
  ...
  let processJsonFunc = (value, response) => {
    ...
    if (value != null) {
      try {
        ...
        logger16.throwError("invalid JSON", Logger.errors.SERVER_ERROR, {
          body: value,
          error
        });
      }
...
var BaseProvider = class extends Provider {
  constructor(network) {
    logger17.checkNew(new.target, Provider);
    super();
    this._events = [];
  ...
  _getResolver(name2) {
    return __awaiter7(this, void 0, void 0, function* () {
      ...
      if (!network.ensAddress) {
        logger17.throwError("network does not support ENS", Logger.errors.UNSUPPORTED_OPERATION, { operation: "ENS", network: network.name });
      }
      const transaction = {
  ...
  resolveName(name2) {
    return __awaiter7(this, void 0, void 0, function* () {
      ...
      if (typeof name2 !== "string") {
        logger17.throwArgumentError("invalid ENS name", "name", name2);
      }
...
function checkError(method, error, params) {
  if (method === "call" && error.code === Logger.errors.SERVER_ERROR) {
    ...
    logger18.throwError("missing revert data in call exception", Logger.errors.CALL_EXCEPTION, {
      error,
      data: "0x"
    });
  }
...
var JsonRpcSigner = class extends Signer {
  constructor(constructorGuard, provider, addressOrIndex) {
    logger18.checkNew(new.target, JsonRpcSigner);
    super();
...
var JsonRpcProvider = class extends BaseProvider {
  constructor(url, network) {
    logger18.checkNew(new.target, JsonRpcProvider);
...
try {
  ...
  const logger31 = new Logger(version10);
  WS = function() {
...
var WebSocketProvider = class extends JsonRpcProvider {
  constructor(url, network) {
    if (network === "any") {
      logger19.throwError("WebSocketProvider does not support 'any' network yet", Logger.errors.UNSUPPORTED_OPERATION, {
  ...
  resetEventsBlock(blockNumber) {
    logger19.throwError("cannot reset events block on WebSocketProvider", Logger.errors.UNSUPPORTED_OPERATION, {
      operation: "resetEventBlock"
    });
  }
  set pollingInterval(value) {
    logger19.throwError("cannot set polling interval on WebSocketProvider", Logger.errors.UNSUPPORTED_OPERATION, {
  ...
  set polling(value) {
    ...
    logger19.throwError("cannot set polling on WebSocketProvider", Logger.errors.UNSUPPORTED_OPERATION, {
...
var UrlJsonRpcProvider = class extends StaticJsonRpcProvider {
  constructor(network, apiKey) {
    logger20.checkAbstract(new.target, UrlJsonRpcProvider);
  ...
  _startPending() {
    logger20.warn("WARNING: API provider does not support pending filters");
  }
  isCommunityResource() {
    return false;
  }
  getSigner(address) {
    return logger20.throwError("API provider does not support signing", Logger.errors.UNSUPPORTED_OPERATION, { operation: "getSigner" });
  }
  listAccounts() {
  ...
  static getUrl(network, apiKey) {
    return logger20.throwError("not implemented; sub-classes must override getUrl", Logger.errors.NOT_IMPLEMENTED, {
      operation: "getUrl"
    });
  }
};
...
var logger23 = new Logger(version10);
...
var EtherscanProvider = class extends BaseProvider {
  constructor(network, apiKey) {
    logger23.checkNew(new.target, EtherscanProvider);
    super(network);
  ...
  getBaseUrl() {
    ...
    return logger23.throwArgumentError("unsupported network", "network", name);
  }
...
init_virtual_process_polyfill();
init_buffer();
var version15 = "random/5.5.1";

// node_modules/@ethersproject/random/lib.esm/random.js
"use strict";
var logger24 = new Logger(version15);
function getGlobal() {
  ...
  for (let i = 0; i < networks2.length; i++) {
    ...
    if (result) {
      if (!(result.name === network.name && result.chainId === network.chainId && (result.ensAddress === network.ensAddress || result.ensAddress == null && network.ensAddress == null))) {
        logger25.throwArgumentError("provider mismatch", "networks", networks2);
      }
    } else {
      result = network;
    }
...
function getRunner(config, currentBlockNumber, method, params) {
  return __awaiter13(this, void 0, void 0, function* () {
    ...
    return logger25.throwError("unknown method error", Logger.errors.UNKNOWN_ERROR, {
      method,
      params
    });
  });
...
var FallbackProvider = class extends BaseProvider {
  constructor(providers, quorum) {
    logger25.checkNew(new.target, FallbackProvider);
    if (providers.length === 0) {
      logger25.throwArgumentError("missing providers", "providers", providers);
    }
    const providerConfigs = providers.map((configOrProvider, index) => {
      ...
      if (weight % 1 || weight > 512 || weight < 1) {
        logger25.throwArgumentError("invalid weight; must be integer in [1, 512]", `providers[${index}].weight`, weight);
      }
    ...
    if (quorum == null) {
      ...
      logger25.throwArgumentError("quorum will always fail; larger than total weight", "quorum", quorum);
    }
    ...
    return __awaiter13(this, void 0, void 0, function* () {
      ...
      while (true) {
        ...
        Object.keys(errors).forEach((errorCode) => {
          ...
          logger25.throwError(e.reason || e.message, errorCode, props);
        });
      ...
      return logger25.throwError("failed to meet quorum", Logger.errors.SERVER_ERROR, {
        method,
        params,
...
init_buffer();
"use strict";
var logger26 = new Logger(version10);
...
var InfuraWebSocketProvider = class extends WebSocketProvider {
  constructor(network, apiKey) {
    ...
    if (connection.password) {
      logger26.throwError("INFURA WebSocket project secrets unsupported", Logger.errors.UNSUPPORTED_OPERATION, {
...
var InfuraProvider = class extends UrlJsonRpcProvider {
  ...
  static getApiKey(apiKey) {
    ...
    if (typeof apiKey === "string") {
      ...
      logger26.assertArgument(typeof apiKey.projectId === "string", "projectSecret requires a projectId", "projectId", apiKey.projectId);
      logger26.assertArgument(typeof apiKey.projectSecret === "string", "invalid projectSecret", "projectSecret", "[REDACTED]");
  ...
  static getUrl(network, apiKey) {
    ...
    switch (network ? network.name : "unknown") {
      ...
      case "arbitrum-rinkeby":
        ...
        break;
      default:
        logger26.throwError("unsupported network", Logger.errors.INVALID_ARGUMENT, {
          argument: "network",
          value: network
        });
    }
...
init_buffer();
"use strict";
var logger27 = new Logger(version10);
...
var NodesmithProvider = class extends UrlJsonRpcProvider {
  ...
  static getUrl(network, apiKey) {
    logger27.warn("NodeSmith will be discontinued on 2019-12-20; please migrate to another platform.");
    let host = null;
    switch (network.name) {
      case "homestead":
...
init_buffer();
"use strict";
var logger28 = new Logger(version10);
...
var PocketProvider = class extends UrlJsonRpcProvider {
  ...
  static getApiKey(apiKey) {
    if (apiKey == null) {
      logger28.throwArgumentError("PocketProvider.getApiKey does not support null apiKey", "apiKey", apiKey);
    }
    const apiKeyObj = {
      applicationId: null,
      loadBalancer: false,
    ...
    if (typeof apiKey === "string") {
      ...
      logger28.assertArgument(typeof apiKey.applicationId === "string", "applicationSecretKey requires an applicationId", "applicationId", apiKey.applicationId);
      logger28.assertArgument(typeof apiKey.applicationSecretKey === "string", "invalid applicationSecretKey", "applicationSecretKey", "[REDACTED]");
      apiKeyObj.applicationId = apiKey.applicationId;
      apiKeyObj.applicationSecretKey = apiKey.applicationSecretKey;
      apiKeyObj.loadBalancer = !!apiKey.loadBalancer;
    } else if (apiKey.applicationId) {
      logger28.assertArgument(typeof apiKey.applicationId === "string", "apiKey.applicationId must be a string", "apiKey.applicationId", apiKey.applicationId);
      apiKeyObj.applicationId = apiKey.applicationId;
      apiKeyObj.loadBalancer = !!apiKey.loadBalancer;
    } else {
      logger28.throwArgumentError("unsupported PocketProvider apiKey", "apiKey", apiKey);
    }
    return apiKeyObj;
  }
  static getUrl(network, apiKey) {
...
init_buffer();
"use strict";
var logger29 = new Logger(version10);
var _nextId = 1;
...
var Web3Provider = class extends JsonRpcProvider {
  constructor(provider, network) {
    logger29.checkNew(new.target, Web3Provider);
    if (provider == null) {
      logger29.throwArgumentError("missing provider", "provider", provider);
    }
    let path = null;
    ...
    if (typeof provider === "function") {
      ...
      if (provider.request) {
        ...
        logger29.throwArgumentError("unsupported provider", "provider", provider);
      }
      if (!path) {
        path = "unknown:";
      }
...
init_buffer();
"use strict";
var logger30 = new Logger(version10);
  ...
  if (typeof network === "string") {
    ...
    if (match) {
      switch (match[1]) {
        ...
        default:
          logger30.throwArgumentError("unsupported URL scheme", "network", network);
      }
    }
  }
  const n = getNetwork(network);
  if (!n || !n._defaultProvider) {
    logger30.throwError("unsupported getDefaultProvider network", Logger.errors.NETWORK_ERROR, {
...
//# sourceMappingURL=chunk-NYT6S7VH.js.map
